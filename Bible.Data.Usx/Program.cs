...
class Program
{
    static void Main()
    {
        ...
        var deserializer = new XmlParser(xmlLogger);

        // Path to your USX XML file
...
<usx version=""3.0"">
  ...
  <chapter eid=""3JN 1"" />
        ...
        logger.LogInformation($"Book: {book.BookMetadata.Code}");
        logger.LogInformation($"Chapter: {book.ChapterMarker?.Number}");

        var stringBuilder = new StringBuilder();
        stringBuilder.AppendLine();
        foreach (var para in book.Paragraphs)
        {
            logger.LogDebug($"Para Style: {para.Style}");
            if (para.Style == "p")
            {
                foreach (var contentItem in para.Content)
                {
                    switch (contentItem)
                    {
                        case string text:
                            ...
                            break;
                        case Link link:
                            logger.LogDebug($"[Link: {link.Text} ({link.Href})]");
                            break;
                        case W w:
                            logger.LogDebug($"[Link: {w.Text} ({w.Lemma})]");
                            break;
                        //case UsxChar usxChar:
                        case IUsxTextBase value:
                            stringBuilder.Append(" ");
                            stringBuilder.Append(value.Text);
                            logger.LogDebug(value.Text);
                            break;
                        case UsxMarker verseMarker:
                            if (verseMarker?.Number != null && verseMarker.StartId != null)
                            {
                                stringBuilder.AppendLine();
                            ...
                            break;
                        case Note note:
                            logger.LogDebug($"[Note: {note}]");
                            break;
                        case Milestone milestone:
                            logger.LogDebug($"[Milestone: {milestone.Style}]");
                            break;
                        default:
                            logger.LogDebug($"[Unknown content type: {contentItem.GetType().Name}]");
                            break;
                    }
                }
